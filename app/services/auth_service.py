...
from app.core.security import decode_refresh_token, hash_password, verify_password
...
class AuthService:
    ...
    async def refresh(self, payload: RefreshTokenRequest) -> AccessTokenResponse:
        """Rotate a valid refresh token and return a fresh token pair."""
        refresh_token = payload.refresh_token.get_secret_value()
        try:
            decoded_payload = decode_refresh_token(refresh_token)
            user_id = uuid.UUID(decoded_payload["sub"])
            user = await self.get_user_by_id(user_id)

            if user is None:
                logger.warning("Refresh token references missing user '%s'.", user_id)
                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED,
            ...
            if not user.is_active:
                logger.warning("Inactive account refresh attempt for user '%s'.", user.id)
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail="User account is inactive",
                )
            ...
            (
                ...
                expires_in,
                refresh_expires_in,
            ) = await AuthSessionService(self.session).rotate_refresh_token(
                user=user,
                refresh_token=refresh_token,
                payload=decoded_payload,
            )
            logger.info("Refresh token rotated successfully for user '%s'.", user.id)
            return AccessTokenResponse(
                access_token=access_token,
                expires_in=expires_in,
                expires_at=auth_session["expires_at"],
            ...
            logger.exception("Database error while rotating refresh token.")
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
